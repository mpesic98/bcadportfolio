...
const creativeFieldCatalog = [
  { key: "display_top", label: "Display Top", hint: "970x90" },
  { key: "display_rail_left", label: "Display Rail Left", hint: "160x600" },
  { key: "display_rail_right", label: "Display Rail Right", hint: "160x600" },
  { key: "display_sidebar", label: "Display Sidebar", hint: "300x250" },
  ...
  { key: "interscroller_video", label: "Interscroller Video", hint: "MP4 / WEBM" },
...
const MAX_LOCAL_CAMPAIGN_BYTES = 2.5 * 1024 * 1024
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-black/10 p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <label className="bc-button bc-button--dark bc-button--sm cursor-pointer">
          Upload
          <input
            type="file"
            accept="image/*,video/*"
...
export default function CampaignsAdminPage() {
  ...
  const uploadFileAsDataUrl = (onValue) => (file) => {
    if (file.size > MAX_LOCAL_UPLOAD_BYTES) {
      setMessage("Local uploads are limited to 512 KB. Use a public asset URL for larger images or videos.")
      return
    }
  ...
  const handleSave = () => {
    ...
    if (
      !selectedCampaignId &&
      campaigns.some((campaign) => campaign.id === nextCampaign.id)
    ) {
      setMessage("That campaign slug already exists. Select it to edit or choose another name.")
      return
    }

    if (new Blob([JSON.stringify(nextCampaign)]).size > MAX_LOCAL_CAMPAIGN_BYTES) {
      setMessage("This local draft is too large. Replace uploaded Data URLs with public asset URLs.")
      return
    }
    ...
    try {
      ...
      setMessage(error.message || "The campaign could not be saved locally.")
      return
    }
    ...
    <div className="grid gap-5 xl:grid-cols-[340px_minmax(0,1fr)]">
      ...
      <section className="rounded-xl border border-white/10 bg-white/5 p-5 md:p-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          {message ? (
            <div className="bc-pill bc-pill--glass">
              {message}
            </div>
          ) : null}
        </div>
        ...
        <div className="mt-6 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-white/58">
              Local paths and uploads work for internal previews. Client exports require public asset URLs.
            </p>
          </div>
          <label className="bc-button bc-button--dark bc-button--sm cursor-pointer">
            Upload logo
